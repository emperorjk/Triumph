...
public class ProductionOverlayMain
{
    ...
    public void OnUpdate()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (!foundBuilding && IsProductionOverlayActive && !CurrentOverlay.GetComponentsInChildren<ProductionScript>().Any(x => x.collider == touchBox.collider))
            {
    ...
    private void CalculateStartPosition()
    {
        ...
        startPosition = new Vector2(centerWorldSpace.x, centerWorldSpace.y - 1);
    }
    ...
    private void CalculateTargetPosition()
    {
        Vector3 centerWorldSpace = Camera.main.ViewportToWorldPoint(new Vector3(0.5f, 0, 0));
        //Vector3 centerWorldSpace = Camera.main.ScreenToWorldPoint(new Vector3(Screen.width / 2, 0, 0));
        targetPosition = new Vector2(centerWorldSpace.x, centerWorldSpace.y + 1);
    }
